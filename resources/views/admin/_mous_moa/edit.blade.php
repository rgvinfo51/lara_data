...
<!-- start page title -->
	...
	<!-- end page title -->
	{!! Form::open(array('route' => ['business.users.update', $user->id],'method'=>'PATCH','files' => 'true','enctype'=>'multipart/form-data')) !!}
 <div class="row">
	<div class="col-lg-12">
		<div class="card">
			 @if (count($errors) > 0)
					...
					<div class="row gy-4">					 
						...
						<div class="col-xxl-3 col-md-6">
							<div>
								...
								<input type="text"  disabled readonly class="form-control" id="labelcode" value="{{Auth::user()->code}}" required>
							</div>
						</div>
						<!--end col-->
						<div class="col-xxl-3 col-md-6">
						...
						<div class="col-xxl-3 col-md-6">
							<div>
								...
								<textarea name="address" class="form-control" id="labelAddress" rows="2">{{old('address', $user->address)}}</textarea>
						...
						<div class="col-xxl-3 col-md-6">
							<div>
								...
								<select name="state" class="form-select" id="labelstate" onchange="get_state_district(this,'{{old('state')}}')">
									...
									<option {{(old('state',$user->state) == $state->id) ? 'selected' : ''}} value="{{$state->id}}">{{$state->state_name}}</option>
									@endforeach									 
								</select>								 
							</div>
						</div>
						...
						<div class="col-xxl-3 col-md-6">
							<div>
								...
								<select name="roles" class="form-select {{$user->type}}" id="roles" required>
									<option value="">Choose...</option>
									@foreach($roles as $role)
										<option {{(old('roles',$user->type) == $role) ? 'selected': ''}} value="{{$role}}">{{$role}}</option>
									@endforeach									 
						...
						<div class="col-xxl-3 col-md-6">
							<div>
								<label for="designation" class="form-label">{{ __('Designation*') }} </label>								 
									<input type="text" name="designation" class="form-control form-control-icon" value="{{old('designation', $user->designation)}}" id="designation" required> 								 
							</div>
						</div>
						<!--end col-->
						<div class="col-xxl-3 col-md-6">
							<div>
								...
								<input type="tel" name="contact_number" maxlength="10" class="form-control numbersonly" value="{{old('contact_number', $user->contact_number)}}" id="labelMobile" required>
							</div>
						</div>
...
@section('script')
<script>
get_state_district('{{$user->state}}','{{$user->district}}');
